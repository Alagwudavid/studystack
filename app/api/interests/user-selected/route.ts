...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await fetch(`${backendUrl}/interests/user-selected`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { success: false, message: data.message || 'Failed to update user interests' },
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { success: false, message: data.message || 'Failed to get user interests' },
                { status: response.status }
            );
        }
