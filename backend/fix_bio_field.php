...
try {
    $pdo = new PDO("pgsql:host={$host};port={$port};dbname={$database}", $username, $password);Fix Bio Field Data
$host = '127.0.0.1';
$username = 'root';
$password = '';
$database = 'bitroot_web';
...
try {
    ...
    if ($issuesFound > 0) {
        ...
        if (strtolower($choice) === 'y' || strtolower($choice) === 'yes') {
            // Clean up bio fields that contain timestamp-like data
            $stmt = $pdo->prepare("UPDATE users SET bio = NULL WHERE bio REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}'");
            $result = $stmt->execute();
